import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import simpy
from message import Message, MessageType
from node import Node, SwitchNode, EndNode
from topos.star import create_star_network
from network import Network

def node_process(env: simpy.Environment, node: Node, network: Network):
    """Main process for each node"""
    env.process(node.receive())
    
    if node.node_id == 1:  # Let node 1 send broadcast message
        yield env.timeout(1)
        print(f"Time {env.now:.2f}: Node 1 starts broadcasting message")
        # Send broadcast message to switch
        yield from network.transmit(1, 0, "Hello everyone!", MessageType.BROADCAST)
        
        # Send another broadcast after some time
        yield env.timeout(5)
        print(f"Time {env.now:.2f}: Node 1 starts broadcasting second message")
        yield from network.transmit(1, 0, "Second broadcast!", MessageType.BROADCAST)

def main():
    # Create simulation environment
    env = simpy.Environment()
    
    # Create star network
    network = create_star_network(
        env=env,
        num_nodes=5,
        bandwidth=100,
        latency=5,
        packet_loss_rate=0.05,
        processing_delay=0.1
    )
    
    # Start processes for all nodes
    for node in network.nodes.values():
        env.process(node_process(env, node, network))
    
    # Run simulation
    print("Starting simulation...")
    env.run(until=20)
    
    # Print statistics
    print("\nStatistics:")
    for node in network.nodes.values():
        node_type = "Switch" if isinstance(node, SwitchNode) else "Node"
        print(f"{node_type} {node.node_id} received {node.messages_received} messages")

if __name__ == "__main__":
    main() 